...
export namespace Record {
	...
	export function create<T>(configuration: Configuration<Record<T>>, target: T): Record<T>
	...
	export function create<T extends globalThis.Record<keyof any, any>>(
		...
		const result = new Proxy<Record<T>>(backend, {
			get(backend: T, p: keyof any, session: Record<T>): T[keyof T] | undefined {
			...
			set(backend: T, p: keyof any, value: T[keyof T], session: Record<T>): boolean {
				const property = p as T[keyof T]
				if (property === "*")
					return false
	...
	export function is(value: unknown): value is Record<globalThis.Record<string | number, unknown>> {
		return typeof value === "object" && !!value && symbol in value && value[symbol] === symbol
	}
	export function entries<T>(
		object: Record<T> | T | undefined
	): [
